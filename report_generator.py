...
with Path.open("diy_coverage_report.txt", "w") as f:
    ...
    for func, branches in coverage_data.items():
        ...
        f.write(
            f"Function coverage: {(covered_branches / total_branches) * 100:.2f}%\n"
        )
        f.write("-" * 45 + "\n\n")
        total_coverage_tracker[0] += covered_branches
        total_coverage_tracker[1] += total_branches
    f.write(
        f"Total coverage: {(total_coverage_tracker[0] / total_coverage_tracker[1]) * 100:.2f}%\n\n"
    )
    f.write("-" * 45)

print("Coverage data written to diy_coverage_report.txt.")
